...
/* =========================================================
   AUDIT STOCK DÉFINITIF
   Un seul moteur pour sauvegarde + audit.
   Aucun ancien wrapper / fallback n'est utilisé.
========================================================= */
...
saveMovement = function(){
    ...
    const reason = getValue("movementReason");
    const note = getValue("movementNote").trim();
    ...
    /*
      IMPORTANT:
      editingMovementId = ID de LA ligne stock réellement modifiée.
      Nouvelle opération => null => nouvel ID unique.
    */
    const editId =
        editingMovementId != null
            ? Number(editingMovementId)
            : null;
    ...
    /*
      Si modification:
      annuler d'abord l'impact de l'ancienne opération
      sur son ancien produit.
    */
    if(editId != null){

        const old = movements.find(
            m => Number(m.id) === editId
        );
        ...
        const oldProduct = products.find(
            p => Number(p.id) === Number(old.productId)
        );
    ...
    /*
      Vérification après annulation de l'ancienne version.
    */
    if(
        type === "exit"
        &&
        ...
        /*
          Restaurer l'ancien impact si l'édition échoue.
        */
        if(before){
            const oldProduct = products.find(
                p => Number(p.id) === Number(before.productId)
            );
    ...
    /*
      Appliquer la nouvelle opération au stock.
    */
    product.stock +=
        type === "entry"
            ? quantity
    ...
    const record = {
        id: operationId,
        auditOperationId: String(operationId),
        date: existingDate,
    ...
    /*
      Sauvegarder exactement LA ligne concernée.
    */
    if(editId != null){
    ...
    /*
      Audit:
      - create => forcément Version 1 de ce movement.id
      - update => version suivante du même movement.id
    */
    const auditId = String(operationId);
    ...
    const label =
        `${labelType} — ${product.name} — ` +
        `${signedQty} ${product.unit || ""}`;

    record.auditCount =
        ppAuditCount("stock", auditId) + 1;

    /*
      IMPORTANT:
      On journalise AVANT de remettre editingMovementId à null.
    */
    ppWriteAudit(
        "stock",
        auditId,
...
/*
  Suppression stock définitive:
  une suppression s'ajoute comme version de LA MÊME opération.
*/
deleteMovement = function(id){
    ...
    if(
        !confirm(
            "Supprimer ce mouvement ? Le stock sera recalculé."
        )
    ){
        return;
    }

    const before = ppAuditSnapshot(movement);
    ...
    const label =
        `${labelType} — ${movement.productName || ""} — ` +
        `${formatNumber(Number(movement.quantity || 0))} ${movement.unit || ""}`;
...
/*
  ...
  uniquement les versions de CETTE opération.
*/
ppAuditMovement = function(id){
